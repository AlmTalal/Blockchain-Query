...
const main = async (blockNumber, address) => {
  //Picking the logs of the Transfer Event (It's the only way i found to view the address who recieves the noun)
  const transferEvent = await contract.queryFilter("Transfer");

  for (let i = 0; i < transferEvent.length; i++) {
    //Discarding the block that are not necesary;
    if (transferEvent[i].blockNumber !== blockNumber) continue;
    //Discarding any other addresses that are in the event
    if (transferEvent[i].args[1] !== address) continue;
    console.log(
      `The address ${address} has the voting rights of the noun ${transferEvent[i].args[2]}`
    );
  }
};

const blockNumber = 16876434;
//decalring the ownerAddress or the address who is gonna get the right to voting
const ownerAddress = "0x008c84421dA5527F462886cEc43D2717B686A7e4";

main(blockNumber, ownerAddress);
